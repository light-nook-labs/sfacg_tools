...
from .config import CORRECT_OCR_SYSTEM_PROMPT, settings
    ...
    def ocr_image(
        ...
        if isinstance(image, (str, Path)):
            ...
            if image_str.startswith(('http://', 'https://')):
                # URL图片
                content.append({'type': 'image_url', 'image_url': {'url': image_str}})
            else:
                # 本地文件
                ...
                if suffix == 'jpg':
                    suffix = 'jpeg'
                content.append({'type': 'image_url', 'image_url': {'url': f'data:image/{suffix};base64,{b64}'}})
