...
class Volumn:
    ...
    def __init__(self, title: str, vol_tag: Tag):
        self.title = title
        self.vol_tag = vol_tag.next_sibling.next_sibling.ul

    def get_volumn_content(self):
        """获取本卷内容"""
        logger.info(f'{self.title}')
        volumn_content = f'## {self.title}\n\n'
        chapters = {}
        ...
        for chapter_title, chapter_url in chapters.items():
            ...
            volumn_content += chapter_content + '\n\n'
        return volumn_content
...
class Novel:
    ...
    def __init__(self, nid: int):
        ...
        self.latest = f'最后更新时间：{self.date} {self.clock}'

    def _get_novel_info(self):
        """获取小说信息"""
        self.index_url = self.base_url_index + str(self.nid)
